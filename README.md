# Emoji Picker Solid | [Live Demo](https://ealush.com/emoji-picker-react)
...
_If you enjoy using emoji-picker-solid<br/>
...
```
npm install emoji-picker-solid
```
...
- `theme`: `Theme` - The theme of the picker. Can be `light`, `dark` or auto. Default is `light`.
  ...
  ```ts
  import { Theme } from 'emoji-picker-solid';
  ```
...
- `emojiStyle`: `EmojiStyle` - The emoji style to use. Can be either `apple`, `google`, `facebook`, `twitter` or `native`. Default is `apple`.
  ...
  ```ts
  import { EmojiStyle } from 'emoji-picker-solid';
  ```
...
```ts
import { SkinTones } from 'emoji-picker-solid';
```
...
- `skinTonePickerLocation`: `SkinTonePickerLocation` - The location of the skin tone picker. Can be either `SEARCH` or `PREVIEW`. Defaults to `SEARCH`. The `SkinTonePickerLocation` enum can be imported from the package.
  ...
  ```ts
  import { SkinTonePickerLocation } from 'emoji-picker-solid';
  ```
...
- `suggestedEmojisMode`: `SuggestedEmojisMode` - The mode to use for the suggested emojis. Can be either `recent` or `frequent`. Defaults to `recent`.
  ...
  ```ts
  import { SuggestionMode } from 'emoji-picker-solid';
  ```
...
```js
import { Emoji, EmojiStyle } from 'emoji-picker-solid';
...
emoji-picker-solid has a top-level error boundary, trying to catch rendering errors. It won't catch server side related errors, or event handlers errors. If an error is caught, the picker will not render, and a console error will be logged.
...
const Picker = lazy(() => import('emoji-picker-solid'));
